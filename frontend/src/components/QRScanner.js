    ...
    try {
      ...
      dispenseEvents.forEach(event => {
        history.push({
          ...
          location: 'Hospital/Pharmacy',
          details: `Dispensed to patient ${event.args.patient ? `${event.args.patient.slice(0, 6)}...${event.args.patient.slice(-4)}` : 'Unknown'}`,
          blockNumber: event.blockNumber,
          transactionHash: event.transactionHash,
          icon: '💊'
        });
                            ...
                            <div className="flex items-center mt-1 text-xs text-gray-500 space-x-4">
                              ...
                              {event.transactionHash && (
                                <span className="truncate">
                                  🔗 {event.transactionHash ? `${event.transactionHash.slice(0, 10)}...` : 'Unknown'}
                                </span>
                              )}
                            </div>
                          </div>
