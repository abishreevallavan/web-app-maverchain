...
import { formatAddress, formatTransactionHash } from '../utils/safeSlice';
...
const AdminDashboard = () => {
  ...
  const formatActivityMessage = (log) => {
    switch (log.type) {
      case 'stock_accepted':
        return `📦 Stock Accepted: ${log.drugName || 'Unknown'} (${log.quantity || 0} units) from ${log.manufacturer || 'Unknown'} by Distributor ${formatAddress(log.distributor)}`;
      case 'manufacturer_request':
        return `🏭 Manufacturer Request: ${log.drugName || 'Unknown'} (${log.quantity || 0} units) requested by Distributor ${formatAddress(log.distributor)}`;
      case 'hospital_request_fulfilled':
        return `🏥 Hospital Request Fulfilled: ${log.drugName || 'Unknown'} (${log.quantity || 0} units) delivered to ${log.hospital || 'Unknown'} by Distributor ${formatAddress(log.distributor)}`;
      case 'batch_created':
        return `🏭 Batch Created: ${log.drugName || 'Unknown'} (${log.quantity || 0} units) by Manufacturer ${formatAddress(log.manufacturer)}`;
      case 'qr_scan':
        return `📱 QR Code Scanned: Batch ${log.batchId || 'Unknown'} scanned by ${formatAddress(log.scanner)}`;
      case 'request_approved':
        return `✅ Request Approved: ${log.drugName || 'Unknown'} (${log.quantity || 0} units) approved for ${formatAddress(log.recipient)}`;
      case 'request_rejected':
        return `❌ Request Rejected: ${log.drugName || 'Unknown'} (${log.quantity || 0} units) rejected for ${formatAddress(log.recipient)}`;
      case 'medicine_approved':
        return `💊 Medicine Approved: ${log.drugName || 'Unknown'} approved by ${formatAddress(log.hospital)}`;
      case 'medicine_declined':
        return `💊 Medicine Declined: ${log.drugName || 'Unknown'} declined by ${formatAddress(log.hospital)}`;
      default:
        return `📋 Activity: ${log.type || 'Unknown'} - ${log.drugName || 'Unknown'}`;
    }
  };
                    ...
                    {getAllSampleAccounts().map((account, index) => (
                      <tr key={index} className="hover:bg-white/5 transition-colors">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-white/70 font-mono">
                          {formatAddress(account.address, 10, 8)}
                        </td>
